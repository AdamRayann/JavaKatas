...
public class Lags {
    ...
    public static int maximizeProfit(List<Request> requests) {
        ...
        return maxReturn(0,requests);

    }
    public static int maxReturn(int end, List<Request> requests) {
        if (requests.isEmpty()) {
            return 0;
        }
        ...
        if (end <= firstRequest.startTime) {
            List<Request> remainingRequests = requests.subList(1, requests.size());

            int includeProfit = firstRequest.payment + maxReturn(firstRequest.startTime + firstRequest.duration, remainingRequests);
            int excludeProfit = maxReturn(end, remainingRequests);

            return Math.max(includeProfit, excludeProfit);
        }

        return maxReturn(end, requests.subList(1, requests.size()));
    }
